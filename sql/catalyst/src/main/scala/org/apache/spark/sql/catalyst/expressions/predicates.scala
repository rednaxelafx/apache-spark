...
import org.apache.spark.sql.catalyst.expressions.codegen.{CodegenConstants, CodegenContext, CodegenUtils, ExprCode, GenerateSafeProjection, GenerateUnsafeProjection, Predicate => BasePredicate}
...
case class In(value: Expression, list: Seq[Expression]) extends Predicate {
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    val javaDataType = CodegenUtils.javaType(value.dataType)
    ...
    val codes = ctx.splitExpressionsWithCurrentInputs(
      ...
      funcName = "valueIn",
      extraArguments = (javaDataType, valueArg) :: (CodegenConstants.JAVA_BYTE, tmpResult) :: Nil,
...
case class InSet(child: Expression, hset: Set[Any]) extends UnaryExpression with Predicate {
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    ev.copy(code =
      s"""
         |${childGen.code}
         |${CodegenConstants.JAVA_BOOLEAN} ${ev.isNull} = ${childGen.isNull};
         |${CodegenConstants.JAVA_BOOLEAN} ${ev.value} = false;
         |if (!${ev.isNull}) {
...
abstract class BinaryComparison extends BinaryOperator with Predicate {
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    if (CodegenUtils.isPrimitiveType(left.dataType)
