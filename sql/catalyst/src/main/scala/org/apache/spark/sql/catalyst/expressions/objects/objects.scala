...
trait InvokeLike extends Expression with NonSQLExpression {
  ...
  def prepareArguments(ctx: CodegenContext): (String, String, String) = {
    ...
    val resultIsNull = if (needNullCheck) {
      val resultIsNull = ctx.addMutableState(CodegenConstants.JAVA_BOOLEAN, "resultIsNull")
      resultIsNull
    } else {
      "false"
    }
    val argValues = arguments.map { e =>
      val argValue = ctx.addMutableState(javaType(e.dataType), "argValue")
      argValue
    }
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    val evaluate = if (returnNullable) {
      if (defaultValue(dataType) == "null") {
        ...
        s"""
          ${boxedType(dataType)} $boxedResult = $callFunc;
    ...
    val code = s"""
      $argCode
      $prepareIsNull
      $javaType ${ev.value} = ${defaultValue(dataType)};
      if (!$resultIsNull) {
        $evaluate
      }
     """
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    val evaluate = if (returnPrimitive) {
      ...
      val assignResult = if (!returnNullable) {
        s"${ev.value} = (${boxedType(javaType)}) $funcResult;"
      } else {
        s"""
          if ($funcResult != null) {
            ${ev.value} = (${boxedType(javaType)}) $funcResult;
          } else {
            ${ev.isNull} = true;
          }
        """
...
case class NewInstance(
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    val code = s"""
      ...
      final $javaType ${ev.value} = ${ev.isNull} ? ${defaultValue(javaType)} : $constructorCall;
    """
    ev.copy(code = code)
  }
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    val code = s"""
      ...
      $javaType ${ev.value} = ${ev.isNull} ?
        ${defaultValue(javaType)} : (${boxedType(javaType)}) ${inputObject.value}.get();
    """
    ev.copy(code = code)
  }
}
...
case class MapObjects private(
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    val elementJavaType = javaType(loopVarDataType)
    ...
    val convertedType = boxedType(lambdaFunction.dataType)
    ...
    val loopNullCheck = if (loopIsNull != "false") {
      ctx.addMutableState(
        CodegenConstants.JAVA_BOOLEAN, loopIsNull, forceInline = true, useFreshName = false)
      inputDataType match {
...
case class CatalystToExternalMap private(
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    val keyElementJavaType = javaType(mapType.keyType)
    ...
    val valueElementJavaType = javaType(mapType.valueType)
    ...
    val valueLoopNullCheck = if (valueLoopIsNull != "false") {
      ctx.addMutableState(CodegenConstants.JAVA_BOOLEAN, valueLoopIsNull, forceInline = true,
        useFreshName = false)
...
case class ExternalMapToCatalyst private(
  ...
  override protected def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    val keyElementJavaType = javaType(keyType)
    val valueElementJavaType = javaType(valueType)
      ...
      case ObjectType(cls) if classOf[java.util.Map[_, _]].isAssignableFrom(cls) =>
        ...
        val defineKeyValue =
          s"""
            ...
            $key = (${boxedType(keyType)}) $entry.getKey();
            $value = (${boxedType(valueType)}) $entry.getValue();
          """
      ...
      case ObjectType(cls) if classOf[scala.collection.Map[_, _]].isAssignableFrom(cls) =>
        ...
        val defineKeyValue =
          s"""
            ...
            $key = (${boxedType(keyType)}) $entry._1();
            $value = (${boxedType(valueType)}) $entry._2();
          """
    ...
    val keyNullCheck = if (keyIsNull != "false") {
      ctx.addMutableState(
        CodegenConstants.JAVA_BOOLEAN, keyIsNull, forceInline = true, useFreshName = false)
    ...
    val valueNullCheck = if (valueIsNull != "false") {
      ctx.addMutableState(
        CodegenConstants.JAVA_BOOLEAN, valueIsNull, forceInline = true, useFreshName = false)
    ...
    val convertedKeyType = boxedType(keyConverter.dataType)
    val convertedValueType = boxedType(valueConverter.dataType)
    val code =
      s"""
        ${inputMap.code}
        ${javaType(dataType)} ${ev.value} = ${defaultValue(dataType)};
...
case class EncodeUsingSerializer(child: Expression, kryo: Boolean)
  ...
  override protected def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    val code = s"""
      ${input.code}
      final $javaType ${ev.value} = ${input.isNull} ? ${defaultValue(javaType)} : $serialize;
     """
...
case class DecodeUsingSerializer[T](child: Expression, tag: ClassTag[T], kryo: Boolean)
  ...
  override protected def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    val code = s"""
      ${input.code}
      final $javaType ${ev.value} = ${input.isNull} ? ${defaultValue(javaType)} : $deserialize;
     """
    ev.copy(code = code, isNull = input.isNull)
  }
...
case class InitializeJavaBean(beanInstance: Expression, setters: Map[String, Expression])
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    val beanInstanceJavaType = javaType(beanInstance.dataType)
...
case class ValidateExternalType(child: Expression, expected: DataType)
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    val typeCheck = expected match {
      ...
      case _ =>
        s"$obj instanceof ${boxedType(dataType)}"
    }
    ...
    val code = s"""
      ...
      if (!${input.isNull}) {
        if ($typeCheck) {
          ${ev.value} = (${boxedType(dataType)}) $obj;
        } else {
